...
getJobsRunning = async() => {
    ...
    await Job.find({ $query: { hasStatus: 'RUNNING' }, $orderby: { dateCreation: 'desc' } }, async(err, jobsRunningDB) => {
...
getContainersOwn = async(userId, jobId) => {
    ...
    await LocalContainer.find({ "User_id": userId, "Job_id": jobId, "Working": false }, async(err, listContainers) => {
        if (listContainers) {
            listContainers.forEach(container => { containersUpdated.push(container) });
        }
    });
    return containersUpdated;
}

// Obtiene los contenedores libres, sin usuario ni job asignado
